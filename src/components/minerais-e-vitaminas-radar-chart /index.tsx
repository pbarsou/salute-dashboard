...
import radarData from '@/data/radar-chart/minerais_e_vitaminas_data.json';

const categories = ['Ferro', 'Cálcio Total', 'Vitamina B12', 'Vitamina D'];

export function MineraisEVitaminasRadarChart() {

  const { theme } = useContext(ThemeContext);

  const darkThemeColors = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF'];
  const lightThemeColors = ['#8884d8', '#82ca9d', '#ffc658', '#d0ed57', '#a4de6c'];
    ...
    <Card className="flex flex-col">
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="leading-none text-muted-foreground">
          Comparação dos resultados dos exames de Minerais e Vitaminas
        </div>
      </CardFooter>
    </Card>
  );
